...
## Fixed in branch `fix/audit-batch2`

| Finding | Severity | Commit | Fix |
|---|---|---|---|
| **F04** | 🟡 MEDIUM | `c5c9988` | `A2Danger.Run` asserts (via `assertAdminLoginable`) that the admin is in `sshusers` **and** has a non-empty `authorized_keys` BEFORE writing the `AllowGroups sshusers` + `PermitRootLogin no` drop-in; either failing returns `StatusFail` "run the PRE step first" instead of leaning on the 300s ssh-revert timer. |
| **F06** | 🟡 MEDIUM | `3ba8c25` | A6.7 apply tags every disk-swap fstab line it comments with a trailing `# morgward-disabled-swap` marker; the revert (`revert.go` `A6.7`) uncomments + strips ONLY marker-tagged lines, so it no longer re-enables swap the operator deliberately disabled before morgward ran. |
| **F07** | 🟡 MEDIUM | `95106df` | A2 revert is now faithful (extend, not warn): besides dropping the sshd drop-ins it `rm`s cloud-init `99-disable-passwords.cfg`, restores `50-cloud-init.conf` `PasswordAuthentication` (no→yes), and `passwd -u root` (best-effort `\|\| true`). Every added action OPENS access, so the lockout-safety invariant holds. |
| **F12** | ⚪ LOW | `ce580d3` | `A2Danger.Run` reordered: the `SwitchUser` handoff now runs while the ssh-revert fail-safe is armed and root is unlocked; only after a proven handoff does it `disarmSSHRevert` + `passwd -l root`. A failed handoff self-heals via the 300s timer with root still usable; failure message says the box self-restores in <300s. |
| **F19** | ⚪ LOW | `b230a61` | `prepare()` captures the `cli.Sudo(writeInventory(...))` result and `log.Warn`s (with the first stderr line) on a non-zero RC instead of discarding it; the write stays non-fatal but a silent miss now surfaces. |
| **F20** | ⚪ LOW | `0951d43` | `cryptoBlock` gates mlkem KEX + PerSourcePenalties strictly on a CONFIRMED `Is2604`; a glitched os-release probe (both flags false) now falls back to the conservative 24.04 set (valid on 26.04 too) instead of emitting OpenSSH-10-only tokens the `syntaxGate` would reject. |
| **F10** | 🟡 MEDIUM | `73922f9` | Security menu renders through a clamped scroll offset; scroll keys/wheel now move it and `secRowYToBodyIdx` adds the same offset, so the bottom DANGER/SAFE buttons are reachable + hittable on a short terminal. |
| **F17** | ⚪ LOW | `0c3fed4` | `keyCopyAtClick` clamps the 'Copy key' button hit-test to the visible scroll region — an off-screen button on a small window no longer reports a stray hit. |
| **F21** | ⚪ LOW | `ef03432`, `bd38939` | §V rows capture the full `Result`; a non-lockout row that hit a transport/exec error (`RC==-1`/`Err!=nil`) now reports `StatusUnknown` "UNMEASURED (<reason>)" instead of a false-RED WARN. Lockout rows still fail closed. Decision core extracted to `classify(Check, Result)` (unit-tested); `Result.Unknown` surfaced as " (N unmeasured)" in the run/verify SUMMARY line. |
| gosec hygiene | LOW | `bfe3224` | G301 log-dir `0o755`→`0o750`; `#nosec` with rationale on G304 (`--key`/`--log` operator paths), G103 (`SetConsoleTitleW` `unsafe.Pointer`), G204/G702 (self-update relaunch — local argv, no shell). Drops 13→7 issues; the 7 remaining are out-of-scope FP/by-design (G704 ssh-agent Dial, G101 i18n/marker, G106 documented `InsecureIgnoreHostKey`). |
| checksums.txt | — | `cfcd403` | `make release` / `build.ps1` / `build.bat` now emit `dist/checksums.txt` (one `sha256  <bare-filename>` line per binary) so the F01 self-update `ChecksumValidator{UniqueFilename:"checksums.txt"}` has an asset to verify against. |
| **F18** (CI) | ⚪ LOW | `7e40e28`, `a1bac99` | `.github/workflows/ci.yml` runs build/vet/test/`go test -race` (CGO_ENABLED=1) + govulncheck on push+PR (Go 1.26.3, ubuntu-latest) — closes the F18 gap (`-race` couldn't run on the Windows host); `a1bac99` pins govulncheck and disables Go `check-latest` for determinism. |

> Out of scope for this branch: **F16** (stale state-persistence contract) was resolved earlier in `6d02f6d` by documenting the checkpoint as in-memory/per-run; **F13** is documentation-only (the 8s `WaitForReboot` sleep is courtesy — `boot_id` inequality is the authoritative reconnect signal), now noted in CLAUDE.md.
