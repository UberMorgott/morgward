...
var docs = map[Lang]map[string]FixDoc{
	RU: {
		"PRE": {
			...
			OnBox:       "Создаёт пользователя (adduser) с группами sudo и sshusers, кладёт ключ в /home/<user>/.ssh/authorized_keys, пишет NOPASSWD-правило /etc/sudoers.d/90-<user>; задаёт консольный пароль (chpasswd).",
		...
		"A2": {
			Title:       "Усиление SSH",
			What:        "Разрешает только современные стойкие алгоритмы шифрования SSH; политику доступа оставляет как в образе (вход по паролю сохранён — не блокирует доступ). Жёсткая блокировка (только ключ, root закрыт) — опционально, через шаг A2-danger в меню безопасности.",
			Why:         "Оставляет только надёжную криптографию, не рискуя заблокировать доступ к серверу по умолчанию.",
			RiskWithout: "SSH принимает слабые, устаревшие шифры.",
		...
		"A10": {
			...
			OnBox:       "Пакеты auditd/audispd-plugins + правила /etc/audit/rules.d/99-vps.rules; PAM-уведомление /usr/local/sbin/ssh-login-notify.sh (строка в /etc/pam.d/sshd); LOG-правила iptables.",
			Revert:      "systemctl disable --now auditd; удалить 99-vps.rules, строку из /etc/pam.d/sshd и ssh-login-notify.sh; убрать LOG-правила (iptables -D INPUT …); автоотката нет.",
		},
	},
	EN: {
		"PRE": {
			...
			OnBox:       "Creates a user (adduser) in the sudo and sshusers groups, installs the key into /home/<user>/.ssh/authorized_keys, writes NOPASSWD rule /etc/sudoers.d/90-<user>; sets a console password (chpasswd).",
		...
		"A2": {
			Title:       "SSH hardening",
			What:        "Allows only modern strong SSH crypto algorithms; leaves the access policy at the image default (password login kept — never locks you out). The hard lockdown (key-only, root blocked) is opt-in via the A2-danger step in the security menu.",
			Why:         "Leaves only robust crypto without risking a default lockout of server access.",
			RiskWithout: "SSH accepts weak, outdated encryption.",
		...
		"A10": {
			...
			OnBox:       "auditd/audispd-plugins packages + rules /etc/audit/rules.d/99-vps.rules; PAM notify hook /usr/local/sbin/ssh-login-notify.sh (line in /etc/pam.d/sshd); iptables LOG rules.",
			Revert:      "systemctl disable --now auditd; remove 99-vps.rules, the line in /etc/pam.d/sshd and ssh-login-notify.sh; drop the LOG rules (iptables -D INPUT …); no auto-rollback.",
		},
	},
}
